...
export function PriceCalculator(){
  ...
  const [window1,setWindow1]=useState(''),[window2,setWindow2]=useState(''),[window3,setWindow3]=useState(''),[ceilingArea,setCeilingArea]=useState('');
  ...
  const result=estimatePrice({rooms:Number(rooms),floor:Number(floor),wall:wall.trim()===''?null:Number(wall),doors:Number(doors),windows1:Number(window1),windows2:Number(window2),windows3:Number(window3),ceilingArea:!ceilings||ceilingArea.trim()===''?null:Number(ceilingArea),ceilings,furnished,colored,condition});
  const range=result&&!result.manual?'CHF '+money(result.low)+'–'+money(result.high):'Individuelle Einschätzung';
  const summary=result ? ['Guten Tag Chau,','ich möchte eine kostenlose Offerte zu dieser Budgetschätzung anfragen.','Zimmer: '+rooms,'Bodenfläche: '+(floor||'nicht angegeben')+' m²','Wandfläche: '+result.wall+' m²'+(result.autoWall?' (geschätzt)':''),'Decken: '+result.ceiling+' m²','Türen mit Zarge: '+(doors||'0'),'Fenster innen (1 / 2 / 3 Flügel): '+[window1||'0',window2||'0',window3||'0'].join(' / '),'Vorbereitung: '+({good:'normaler Untergrund',light:'leichte Ausbesserungen und Grundierung',full:'Wände vollflächig spachteln und streichen',damaged:'starke Schäden / Feuchtigkeit / Nikotin'}[condition]),'Wände farbig: '+(colored?'ja':'nein'),'Möbliert: '+(furnished?'ja':'nein'),'Unverbindliches Planungsbudget: '+range,'Bitte prüfen Sie den tatsächlichen Aufwand.','Mein Ort:','Gewünschter Zeitraum:','Fotos füge ich dieser E-Mail bei.'].join('\n') : '';
  ...
  return <section className="section price-calculator" id="kosten" aria-labelledby="price-heading">
    <div className="price-heading"><p className="eyebrow">Erste Orientierung · Schweizer Richtpreise</p><h2 id="price-heading">Was kostet mein Malerprojekt?</h2><p>Tragen Sie nur die gewünschten Arbeiten ein. Wandfläche, Decke, Türen und Fenster lassen sich einzeln oder gemeinsam berechnen. Sie sehen sofort eine unverbindliche Kostenspanne in Schweizer Franken.</p></div>
    <div className="price-layout">
      <div className="price-fields">
        ...
        <div className="price-input-grid" style={{marginTop:22}}>
          <label htmlFor="calc-window1">Fenster innen · 1 Flügel<Input id="calc-window1" type="number" min="0" max="30" step="1" value={window1} placeholder="0" onChange={e=>setWindow1(e.target.value)}/></label>
          <label htmlFor="calc-window2">Fenster innen · 2 Flügel<Input id="calc-window2" type="number" min="0" max="30" step="1" value={window2} placeholder="0" onChange={e=>setWindow2(e.target.value)}/></label>
          <label htmlFor="calc-window3">Fenster innen · 3 Flügel<Input id="calc-window3" type="number" min="0" max="30" step="1" value={window3} placeholder="0" onChange={e=>setWindow3(e.target.value)}/></label>
        </div>
      ...
      <div className="price-result">
        ...
        {!result?<p>Mindestens eine Fläche oder Stückzahl eingeben. Bodenfläche ist nicht erforderlich. Zulässig: bis 2’000 m² Wandfläche, bis 500 m² Boden-/Deckenfläche und je 0–30 Türen/Fenster. Bei aktivierten Decken bitte Decken- oder Bodenfläche angeben.</p>:result.manual?<p>Stark beschädigte Wand- oder Deckenflächen benötigen eine persönliche Einschätzung. Senden Sie mir ein paar Fotos.</p>:<>
          <p className="price-note">{result.wall>0?result.wall+' m² Wände. ':''}{result.ceiling>0?result.ceiling+' m² Decken. ':''}{result.autoWall?'Wandfläche automatisch geschätzt.':''} Nur eingetragene Arbeiten werden berechnet.</p>
