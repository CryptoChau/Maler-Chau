...
export function PriceCalculator(){
  ...
  return <section className="section price-calculator" id="kosten" aria-labelledby="price-heading">
    ...
    <div className="price-layout">
      ...
      <div className="price-result">
        ...
        {!result?<p>Mindestens eine Fläche oder Stückzahl eingeben. Bodenfläche ist nicht erforderlich. Zulässig: bis 2’000 m² Wand-/Fassadenfläche, bis 500 m² Boden-/Deckenfläche und je 0–30 Türen/Fensterflügel. Bei aktivierten Decken bitte Decken- oder Bodenfläche angeben.</p>:result.manual?<p>Stark beschädigte Wand- oder Deckenflächen benötigen eine persönliche Einschätzung. Senden Sie mir ein paar Fotos.</p>:<>
          ...
          <dl className="price-breakdown">{result.lines.filter(r=>r.high>0).map(r=><div key={r.label}><dt>{r.label}</dt><dd>CHF {money(r.low)}–{money(r.high)}</dd></div>)}<div><dt>Übliche Abdeckarbeiten &amp; Schutzmaterial</dt><dd>Inklusive</dd></div><div><dt>Standardwerkzeug &amp; übliche Geräte</dt><dd>Inklusive</dd></div></dl>
    ...
    <details className="price-method"><summary>Preisgrundlagen & enthaltene Annahmen</summary>
      ...
      <p>Rechnerannahmen: Die Arbeitspreise enthalten Standardmaterial, übliche Abdeckarbeiten mit Schutzmaterial sowie die Nutzung von Standardwerkzeug und üblichen Geräten (zum Beispiel Pinsel und Rollen). Diese Positionen werden nicht zusätzlich aufgeschlagen. Spezialgeräte wie Hebebühnen und besonderer Schutzaufwand werden nach Bedarf separat offeriert. Bei Möblierung werden auf Innenarbeiten 10–20 % Reserve für zusätzlichen Schutzaufwand und erschwerten Zugang ergänzt; dies ist eine Planungsannahme, kein erhobener Schweizer Durchschnitt. Decken werden mit normalem Untergrund gerechnet. Spezialfarben, Tapetenentfernung, Schimmel, Gerüste, aufwendige Reparaturen, Räumungen und besondere Anfahrten sind nicht enthalten.</p>
